...
public class ChainedRuntimeExceptionTest extends TestCase {
   ...
   */
  public ChainedRuntimeExceptionTest(String name) {
    super(name);
  }
   ...
   */
  private void rethrowExceptionOnce() {
    try {
      provokeException();
    } catch (Exception e) {
      throw new TestChainedRuntimeException("mein Fehlertext innen", e);
    }
  }
   ...
   */
  private void rethrowExceptionTwice() {
    try {
      rethrowExceptionOnce();
    } catch (Exception e) {
      throw new TestChainedRuntimeException("mein Fehlertext aussen", e);
    }
  }
  ...
  public void testPrintStackTrace() {
    ...
    String testChainedRuntimeException = TestChainedRuntimeException.class.getName();
    String chainedRuntimeExceptionTest = this.getClass().getName();

    List expectedOutput = new ArrayList();
    expectedOutput.add("    " + testChainedRuntimeException + ": mein Fehlertext aussen");
    expectedOutput.add("  " + testChainedRuntimeException + ": mein Fehlertext innen");
    expectedOutput.add("java.lang.NullPointerException");
    expectedOutput.add("at " + chainedRuntimeExceptionTest + ".provokeException(");
    expectedOutput.add("  at " + chainedRuntimeExceptionTest + ".rethrowExceptionOnce(");
    expectedOutput.add("    at " + chainedRuntimeExceptionTest + ".rethrowExceptionTwice(");
    expectedOutput.add("    at " + chainedRuntimeExceptionTest + ".testPrintStackTrace(");

    try {

      rethrowExceptionTwice();

    } catch (ChainedRuntimeException cre) {

      if (CONSOLE_OUTPUT) { cre.printStackTrace(); }
      
      StringWriter sw = new StringWriter();
      PrintWriter pw = new PrintWriter(sw);
      cre.printStackTrace(pw);
      pw.close();
